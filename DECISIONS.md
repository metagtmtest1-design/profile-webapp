...
| D-28 | Calendar Slots Backend | GET /api/calendar/slots?weeks=2 — computeSlotsForDay Eastern wall time to UTC ISO via getEasternOffsetHours Intl longOffset GMT-04:00 + fallback DST, TIMEZONE=America/New_York constant, workingHours 09-17 Mon-Fri days 1-5, slotMinutes via normalizeSlotMinutes multiple 15 (15,30,45,60 valid, round down), busy blocks exclude via overlap slotStart < busyEnd && slotEnd > busyStart, getNext14Range + getSunday/getSaturday max 3 weeks Sun-Sat, selectableSet only next 14 days, getStubSlots fallback, getFreeBusy SA JWT RS256 SubtleCrypto | Eastern timezone for now per requirement, configurable later via TIMEZONE admin var, data isolated alpha vs prod via BOOKING var | 2026-07-23 |
| D-29 | Calendar Exclude Today | EXCLUDE_TODAY=true default for all envs local/preview/production (was false) via wrangler.toml + code defaults true everywhere (CalendarView default false→true, useCalendar useState false→true, slots.ts parseExcludeToday default true via ?? 'true', getStubSlots fallback true) — option not taking any schedule today, assume we dont schedule today for now | Requirement 2 of Slice 2 polish: not taking bookings today — from tomorrow | 2026-07-23 |
| D-30 | Calendar UI Premium | CalendarView from full month grid getDaysInMonth 28-31 buttons + month nav (audit 32% confidence) redesigned to 3-week Sun-Sat grid max 21 days only 14 selectable (7 per row ×2-3 rows) via getCalendarGrid, weekday header Sun Mon Tue etc, Today badge px-4 py-1.5 not close to border (was px-1.5 py-0.5 9px), 30m badge removed repetition kept just text, min-h 92/96 prevents 16 slots badge cut off, no -mx-2 overflow causing dates not within box, floating text blocking fixed by Home vertical layout calendar top + SlotPicker below expand under not side, SlotPicker smaller buttons px-3 py-2.5 text-xs border-slate-200 gap-3 hover:border-slate-900 hover:z-10 no overlap on hover (was px-4 py-3 overlapping 5:30-6:00 screenshot), interval 9:00-9:30 via formatSlotTimeLocal stripping AM + formatSlotInterval, timezone Eastern America/New_York via TIMEZONE constant (was UTC then local), close button ✕ w-9 h-9 + onClose, remove privacy note | Fixes screenshots 1-2 + user reports buttons text close to border (px-7 undefined) + Today/30m badges close + cannot close modal + 3-week requirement | 2026-07-23 |
| D-31 | Branch Naming Slice 2 | slice2-1-calendar-slots, slice2-2-premium-calendar, slice2-3-calendar-fix, slice2-4-calendar-weeks, slice2-5-clean-calendar, slice2-6-eastern-timezone — later contains previous superset, only latest needs PR to alpha per convention slice# + commitcount + words | Per new naming convention | 2026-07-23 |
| D-32 | Timezone | Use Eastern America/New_York for now, configurable in admin via TIMEZONE var later (future admin can set TIMEZONE), display and generation both in same TZ, visitor in different timezone converted via Eastern display (not visitor local for now) | Per user request for now use Eastern | 2026-07-23 |

## Current Status — Slice 2 ✅ Complete (Calendar Slots + Premium UI)

- **Deployed Local:** Docker Compose backend auto-migrates D1 DB --local, /api/health db ok r2 ok env local (both D1+R2), /api/content/home home 6 sections 18 items, /api/calendar/slots?weeks=2 weeks 2 source stub 144 slots from tomorrow (EXCLUDE_TODAY=true) workingHours slotMinutes 30 multiple 15 excludeToday true timezone ET
- **Deployed Remote Alpha:** https://alpha.profile-webapp.pages.dev/api/health env alpha db ok r2 ok, .../api/content/home home 6 real D1 (after --env preview seed), .../api/calendar/slots?weeks=2 source live when GCAL_SERVICE_ACCOUNT_KEY secret + BOOKING alpha 4b32... + PERSONAL metagtmtest1@gmail.com as Encrypted Secrets via Dashboard Preview, stub otherwise, calendar UI max 3 weeks Sun-Sat 7 per row only next 14 selectable from tomorrow, badges padded, close button, interval 9:00-9:30 ET smaller buttons no overlap, no privacy note, no blog/login, combined Tristan CPA premium with calendar
- **Deployed Prod:** https://profile-webapp.pages.dev same after PR alpha→main, env production db ok r2 ok
- **Tests:** 43 FE +53 BE =96 green via Docker node:20, build CSS 9.04KB JS 175KB
- **Docs:** doc/Setup.md 529 lines general setup guide with GCP + SA + 2 calendars + sharing + PII as Secrets + 3-week calendar + branch control screenshot + token fetch, plus this DECISIONS.md updated to D-32, README updated to Slice 1-2 complete, wrangler.toml has EXCLUDE_TODAY=true + SLOT 30 multiple 15 + TIMEZONE future
- **Next:** Slice 3 Booking POST /api/booking ⭐

Next: Slice 3 Booking POST /api/booking ⭐ core
