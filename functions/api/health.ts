...
export const onRequestGet: PagesFunction<Env> = async ({ env }) => {
  ...
  // Both D1 and R2 are required for all envs (alpha uses alpha D1+R2, prod uses prod D1+R2)
  // R2 is now enabled (billing active) — must be checked for both preview (alpha) and production (prod)
  ...
  // Both D1 and R2 must be ok for healthy
