...
describe('POST /api/booking — full 10-step workflow', () => {
  ...
  it('should return 400 when Turnstile verification fails (token missing/invalid)', async () => {
    ...
    const request = new Request('http://localhost:8788/api/booking', {
      ...
      body: JSON.stringify({ firstName: 'John', lastName: 'Doe', email: 'john@example.com', slot: { start: '2026-07-30T13:00:00Z', end: '2026-07-30T13:30:00Z' }, turnstileToken: '' }),
    })
  ...
  it('should return warning flag same email booked this week', async () => {
    ...
    const request = new Request('http://localhost:8788/api/booking', {
      ...
      body: JSON.stringify({
        ...
        slot: { date: '2026-07-30', start: '2026-07-30T13:00:00Z', end: '2026-07-30T13:30:00Z', available: true },
        purpose: 'Follow up',
        turnstileToken: 'fake',
      }),
    })
  ...
  it('should handle invalid email format 400 and missing fields', async () => {
    ...
    const reqInvalidEmail = new Request('http://localhost:8788/api/booking', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ firstName: 'John', lastName: 'Doe', email: 'not-an-email', slot: { start: '2026-07-30T13:00:00Z', end: '2026-07-30T13:30:00Z' }, turnstileToken: 'fake' }),
    })
